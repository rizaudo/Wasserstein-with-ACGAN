...
class NonLabelCritic(chainer.Chain):
    def __init__(self, ch=512, bottom_width=4, wscale=0.02, output_dim=1):
...
class Classfier(chainer.Chain):
    def __init__(self, y_dim=10):
        ...
        size = 32
        with self.init_scope():
            self.c0 = L.Convolution2D(3, size, ksize=5, stride=1)
...
class Updater(chainer.training.StandardUpdater):
    ...
    def update_core(self):
        # NOTE: uncondtionalになるのは、batch dataと同じラベルをGeneratorに食わせてるからなのでは。
        ...
        for i in range(self.n_critic):
            ...
            e = xp.random.uniform(0., 1., (batchsize, 3, 32, 32)).astype(np.float32)
